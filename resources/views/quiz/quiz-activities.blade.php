				...
				<div class="card mt-2" style="min-height: 600px;">
					...
					<div class="card-body">
						<div class="ms-2 mt-2 pb-2 ps-5">
                            ...
                            background: {{ $activity['cover_image'] ? 'url('.$activity['cover_image'].') center/cover no-repeat' : '' }};
                            color: #fff; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.7);">
									<div class="d-flex flex-column justify-content-between align-items-center" style="height: 100px;">
										<div class="w-100 d-flex justify-content-center">
											<a href="{{ route('load-game-in-page',[ $activity['id']]) }}" class="no_decoration" style="color: #fff; text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.7);">{{ $activity['title'] }}</a>
										</div>
										<div class="w-100 d-flex align-items-end">
											<div class="w-100 d-flex justify-content-center align-items-end">
												<a href="{{ route('quiz-builder',[ $activity['id']]) }}" class="no_decoration btn btn-info me-2"><i class="fas fa-edit"></i></a>
												<a href="{{ route('quiz-activities-action',['clone',$activity['id']]) }}" class="no_decoration btn btn-info me-2"><i class="fas fa-copy"></i></a>
												<span class="no_decoration btn btn-info delete_activity" data-id="{{ $activity['id'] }}" style="cursor: pointer;"><i class="fas fa-trash"></i></span>
											</div>
										</div>
									</div>
								</div>
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-danger">
						<a href="{{ route('quiz-activities-action',['delete','']) }}" id="delete-activity-confirm" class="no_decoration">{{ __('default.Delete') }}</a>
					</button>
